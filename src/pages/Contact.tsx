...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    emailjs.sendForm(
      ...
      .then(
        () => {
          setStatus(t('Message envoyé ✅', 'Message sent ✅'));
          formRef.current?.reset()
        },
        () => {
          setStatus(t("Erreur lors de l'envoi ❌", 'Error sending message ❌'));
        }
      )
    ...
    >
      ...
      >
        {t(
          ...
          "Stop searching, you've found the best. Send your message and watch the magic happen."
        )}
      </motion.p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          <input
            ...
            type="email"
            name="email"
            placeholder={t("Ton email", "Your email")}
            className="w-full p-3 rounded-md bg-transparent border border-[#f2cc6a] outline-none"
          />
          <textarea
            required
        ...
        >
          <div className="absolute bottom-4 right-4 flex flex-col space-y-4 text-white/70">
            ...
            >
              <span className="absolute left-0 top-0 h-full w-12 bg-linear-to-r from-[#f2cc6a] via-[#f2cc6a] to-white/90 rounded-xl transition-all duration-300 ease-out group-hover:w-full" />
              <a href="https://wa.me/237673846813" className="relative flex items-center gap-3 z-10 text-white transition-colors group-hover:text-white/90">
            ...
            >
              <span className="absolute left-0 top-0 h-full w-12 bg-linear-to-r from-[#f2cc6a] via-[#f2cc6a] to-white/90 rounded-xl transition-all duration-300 ease-out group-hover:w-full" />
              <a href="https://www.instagram.com/jp_graphic_design/" className="relative flex items-center gap-3 z-10 text-white transition-colors group-hover:text-white/90">
                <div className="relative z-10 w-12 h-12 flex items-center justify-center rounded-xl">
            ...
            >
              <span className="absolute left-0 top-0 h-full w-12 bg-linear-to-r from-[#f2cc6a] via-[#f2cc6a] to-white/90 rounded-xl transition-all duration-300 ease-out group-hover:w-full" />
              <a href="tel:+237659980127" className="relative flex items-center gap-3 z-10 text-white transition-colors group-hover:text-white/90">
                <div className="relative z-10 w-12 h-12 flex items-center justify-center rounded-xl">
